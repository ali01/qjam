...
% TODO(ms): This section needs to be entirely rewritten.
%
% Some things that might be worth mentioning:
%   - define worker-local storage
%   - why is it important?
%     Iterations can be short-lived, and often need to use old data. This data
%     needs to be accessed fast, so in-memory storage for low-latency access is
%     key. This is one of the reasons we didn't use hadoop.)
%   - how the worker gets the data
%     it compares the refs it receives in a task message, asks for those it
%     doesn't have
...
%     TODO(sqs): list the names of everyone in our meeting group (Bobby, Sisi,
%     Milinda, etc.)
